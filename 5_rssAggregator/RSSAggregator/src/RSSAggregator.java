...
public final class RSSAggregator {
    ...
    private static void processFeed(String url, String file, SimpleWriter out) {
        ...
        if (rss.label().equals("rss") && rss.hasAttribute("version")
            ...
            fileOut.close();
            out.println("HTML file created: " + file);
        } else {
    ...
    public static void main(String[] args) {
        ...
        out.print("Enter the name of the XML file with RSS feed URLs: ");
        String xmlFileName = in.nextLine();
        out.print("Enter the name of the output index file: ");
        String indexFileName = in.nextLine();

        if (!xmlFileName.isEmpty() && !indexFileName.isEmpty()) {
            if (!indexFileName.endsWith(".html")) {
                indexFileName += ".html";
            }

            XMLTree feeds = new XMLTree1(xmlFileName);
            ...
            indexOut.println("<head><title>" + indexTitle + "</title></head>");
            indexOut.println("<body><h1>" + indexTitle + "</h1>");
            indexOut.println("<ul>");

            for (int i = 0; i < feeds.numberOfChildren(); i++) {
                if (feeds.child(i).label().equals("feed")) {
                    String feedUrl = feeds.child(i).attributeValue("url");
                    String feedName = feeds.child(i).attributeValue("name");
                    String htmlFileName = feeds.child(i).attributeValue("file");

                    out.println("Processing feed: " + feedName + " (" + feedUrl + ")");

                    // Process each feed
                    processFeed(feedUrl, htmlFileName, out);

                    // Add link to the index
                    indexOut.println("<li><a href=\"" + htmlFileName + "\">" + feedName
                            + "</a></li>");
                }
            }
            ...
            indexOut.close();
            // Check the state
            out.println("Index file created: " + indexFileName);
        } else {
            out.println("Invalid input. Please provide valid file names.");
        }
